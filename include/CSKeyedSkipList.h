...
public:
  ...
  typedef KeyedSkipList<K,T,Pr,R> container_type;
  ...
  {
  friend class KeyedSkipList<K,T,Pr,R>;
  public:
...
#define csarg2 KeyedSkipList<K,T,Pr,R>
...
template <class K, class T, class Pr = std::less<K>, class R = RNG >
class MultiKeyedSkipList
{
public:
  typedef CSUNIQUE(CSKEY(uniquekey_tag,unique_tag),CSKEY(multikey_tag,multi_tag)) tag;
  typedef MultiKeyedSkipList<K,T,Pr,R> container_type;
  typedef BidiIterator<container_type> T0;
  typedef ConstBidiIterator<container_type> T1;
  friend class BidiIterator<container_type>;
  friend class ConstBidiIterator<container_type>;
  ...
  class value_compare
    : public std::binary_function<value_type, value_type, bool>
  {
  friend class MultiKeyedSkipList<K,T,Pr,R>;
  public:
    bool operator()(const value_type& left, const value_type& right) const
      {return (comp(left.first, right.first)); }
  protected:
...
public:
  ...
  MultiKeyedSkipList() : ValueCompare(Pr()) { CSInitDefault; }
  explicit MultiKeyedSkipList(size_type maxNodes) : ValueCompare(Pr()) { CSInitMaxNodes; }
  MultiKeyedSkipList(double probability, size_type maxLevel) : ValueCompare(Pr()) { CSInitPM; }
  MultiKeyedSkipList(const container_type &source) : ValueCompare(source.ValueCompare), KeyCompare(source.KeyCompare) { CSInitCore(source.probability, source.maxLevel) CSCopyITIT(const_iterator, source.begin(),source.end(),insert); }
  template<class InIt> MultiKeyedSkipList(InIt first, InIt last) : ValueCompare(Pr()) { CSInitDefault; CSCopyITIT(InIt, first,last,insert); }
  template<class InIt> MultiKeyedSkipList(InIt first, InIt last, double probability, size_type maxLevel) : ValueCompare(Pr()) { CSInitPM; CSCopyITIT(InIt, first,last,insert); }
  template<class InIt> MultiKeyedSkipList(InIt first, InIt last, size_type maxNodes) : ValueCompare(Pr()) { CSInitMaxNodes; CSCopyITIT(InIt, first,last,insert); }
  explicit MultiKeyedSkipList(const key_compare& comp) : ValueCompare(comp), KeyCompare(comp) { CSInitDefault; }
  template<class InIt> MultiKeyedSkipList(InIt first, InIt last, const key_compare& comp) : ValueCompare(comp), KeyCompare(comp) { CSInitDefault; CSCopyITIT(InIt, first,last,insert); }
  template<class InIt> MultiKeyedSkipList(InIt first, InIt last, const key_compare& comp, double probability, size_type maxLevel) : ValueCompare(comp), KeyCompare(comp) { CSInitPM; CSCopyITIT(InIt, first,last,insert); }
  template<class InIt> MultiKeyedSkipList(InIt first, InIt last, const key_compare& comp, size_type maxNodes) : ValueCompare(comp), KeyCompare(comp) { CSInitMaxNodes; CSCopyITIT(InIt, first,last,insert); }
  ~MultiKeyedSkipList() { clear(); }
  CSDefineOperatorEqual
...
#define csarg1 template<class K, class T, class Pr, class R>
#define csarg2 MultiKeyedSkipList<K,T,Pr,R>
CSDefineCompOps(csarg1, csarg2)
#undef csarg1
#undef csarg2
